  ...
  return (
    <main className="mx-auto max-w-3xl px-5 py-10 sm:px-8 sm:py-14">
      ...
      <header className="mt-8 border-b border-border pb-8">
        <p className="font-mono text-[10px] font-semibold uppercase tracking-[0.16em] text-brand">New content</p>
        <h1 className="mt-3 text-4xl font-medium tracking-[-0.06em] text-navy sm:text-5xl">Create article</h1>
        <p className="mt-4 max-w-xl font-mono text-xs leading-6 text-foreground-muted">Start with the essentials. You can continue refining the article after it is saved.</p>
      </header>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="mt-8 space-y-7" noValidate>
        <div className="space-y-5 border border-border bg-surface p-5 sm:p-6">
          <Field data-invalid={Boolean(errors.title)}>
            ...
            <Input
              ...
              {...register("title", {
                ...
                onBlur: (event) => {
                  if (!getValues("slug")) {
                    setValue("slug", createSlug(event.target.value), { shouldValidate: true });
                  }
                },
              })}
            />
          ...
          <Field data-invalid={Boolean(errors.description)}>
            ...
            <Textarea id="article-description" placeholder="A short summary for article lists and search previews." aria-invalid={Boolean(errors.description)} disabled={isSubmitting} {...register("description", { maxLength: { value: 320, message: "Description must be 320 characters or fewer." } })} />
          ...
          <Field data-invalid={Boolean(errors.coverImage)}>
            ...
            <Input id="article-cover-image" type="url" placeholder="https://example.com/cover.jpg" aria-invalid={Boolean(errors.coverImage)} disabled={isSubmitting} {...register("coverImage", { validate: (value) => !value || /^https?:\/\//i.test(value) || "Enter a full http:// or https:// URL." })} />
        ...
        <div className="border border-border bg-surface p-5 sm:p-6">
          <Field data-invalid={Boolean(errors.body)}>
            ...
            <FieldDescription>This first version saves the body as a paragraph content block.</FieldDescription>
            <Textarea id="article-body" className="min-h-56" placeholder="Write the opening of your article..." aria-invalid={Boolean(errors.body)} disabled={isSubmitting} {...register("body", { required: "Article body is required.", validate: (value) => value.trim().length > 0 || "Article body is required." })} />
        ...
        <label className="flex cursor-pointer items-start gap-3 border border-border bg-surface p-5 text-sm text-navy has-[:focus-visible]:outline-2 has-[:focus-visible]:outline-offset-4 has-[:focus-visible]:outline-brand">
          ...
          <span>
            ...
            <span className="mt-1 block font-mono text-[10px] leading-5 text-foreground-muted">Leave this unchecked to save the article as a draft.</span>
          </span>
        </label>
